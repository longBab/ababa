      ...
      <view class="page">
        ...
        <view class="content">
          ...
          <view>
            <view class="list" v-if="active == 0">
              <view
                ...
                :key="index"
                @click="_toQrCode(item)"
              >
                <image :src="couponImg" class="coupon"></image>
                <view class="box">
                  <view class="descripe">
                    <view class="shop-name"
                      >{{ item.couponName }}({{ item.stock }}张)</view
                    >
                    <view class="text">{{ item.value }}</view>
                    <view class="expire"
                      >{{ item.createTime }}-{{ item.endTime + " 前" }}</view
                    >
                  </view>
                  <view class="usestate">
                    {{ item.toTypeName }}
          ...
          <view class="list expired" v-if="active == 1">
            <view
              class="listItem"
              v-for="(item, index) in useCouponList"
              :key="index"
            >
              <image :src="couponImg" class="coupon"></image>
              <view class="box">
                <view class="descripe">
                  ...
                  <view class="text">{{ item.value }}</view>
                  <view class="expire"
                    >{{ item.createTime }}-{{ item.endTime + "00：00" }}</view
                  >
                </view>
                <view class="usestate"> 已使用 </view>
              </view>
            </view>
          </view>
          <!-- 已过期 -->
          <view class="list expired" v-if="active == 2">
            <view>
              <view
                class="listItem"
                v-for="(item, index) in tmpCouponList"
                :key="index"
              >
                <image :src="couponImg" class="coupon"></image>
                <view class="box">
    ...
    methods: {
      ...
      _toQrCode: function (_coupon) {
        if (_coupon.toType == "2002") {
          ...
          uni.navigateTo({
            url:
              "/pages/coupon/giftParkingCoupon?couponId=" +
              _coupon.couponId +
              "&couponName=" +
              _coupon.couponName,
          });
