  ...
  <view>
    ...
    >
      <swiper-item class="swiper-items">
          ...
          <view class="coupins-items">
            <radio-group>
              <view
                :class="
                  'coupins-item ' +
                  (item.couponsMemberRelationMap.isRelation ? '' : 'isRelation')
                "
              ...
              >
                <view class="item-top">
                  <view>
                    <view class="couponsName">
                      {{ item.couponsMemberRelationMap.couponsName }}-{{
                        item.couponsMemberRelationMap.preferentialTypeText
                      }}
                    </view>
                    <view class="endTime"
                      >有效期至{{ item.couponsMemberRelationMap.endTime }}</view
                    >
                  </view>
                    ...
                    <view class="couponsNum">
                      {{
                        item.couponsMemberRelationMap.preferentialType == 1
                          ? item.couponsMemberRelationMap.discountAmount * 10 +
                            "折"
                          : "满" +
                            item.couponsMemberRelationMap.limitedPrice +
                            "减" +
                            item.couponsMemberRelationMap.reducedPrice
                      }}
                    </view>
                    <view
                      v-if="
                        item.couponsMemberRelationMap.afterDiscountPrice >= 0 &&
                        item.couponsMemberRelationMap.isRelation
                      "
                      class="afterDiscountPrice"
                    >
                      优惠{{
                        item.couponsMemberRelationMap.afterDiscountPrice
                      }}元
                    </view>
                  </view>
                </view>
                ...
                <view class="bottom-view">
                  <view class="usage-rule" @tap="onClick" :data-index="index">
                    使用规则
                    <text
                      class="iconfont iconweibiaoti35"
                      v-if="item.couponsMemberRelationMap.isShow"
                    ></text>
                    <text
                      class="iconfont iconhtbArrowright02"
                      v-if="!item.couponsMemberRelationMap.isShow"
                    ></text>
                  </view>
                  <view
                    :class="
                      'immediate-use ' +
                      (!item.couponsMemberRelationMap.isExpired &&
                      !item.couponsMemberRelationMap.isUsed &&
                      item.couponsMemberRelationMap.isRelation
                        ? 'theme-color-border'
                        : 'out-of-commission') +
                      ' '
        ...
        <Empty
          :description="'暂无' + tabList[0].modeName"
          v-if="couponsList.length <= 0"
        />
      </swiper-item>
      <swiper-item class="swiper-items">
            ...
            <radio-group>
              <view
                :class="
                  ...
                  (item.couponsMemberRelationMap.isRelation ? '' : 'isRelation')
                "
                v-for="(item, index) in couponsPointsList"
                :key="index"
              >
                <view class="item-top">
                  <view>
        ...
        <Empty
          :description="'暂无' + tabList[1].modeName"
          v-if="couponsPointsList.length <= 0"
        />
      </swiper-item>
    </swiper>
...
export default {
  ...
  methods: {
        ...
        success: function (res) {
          ...
          if (that.prevData) {
            ...
            query.exec(function (res) {
              ...
              that.winHeight = winHeight - res[0].height;
            });
          }
        },
      });
    ...
    getCouponsMemberRelation() {
      ...
      https
        .request("/rest/member/couponsMemberRelation/list", {
          pageNo: pageNo,
          pageSize: pageSize,
          isUsed: 0,
          isExpired: 0,
        ...
        .then((result) => {
          ...
          if (result.success) {
            result.data.records.forEach((res) => {
              res.couponsMemberRelationMap.preferentialTypeText =
                systemStatus.preferentialTypeText(
                  res.couponsMemberRelationMap.preferentialType
                );
              res.couponsMemberRelationMap.startTime = dateHelper.formatISODate(
                res.couponsMemberRelationMap.startTime,
                "YYYY-MM-DD"
              );
              //res.couponsMemberRelationMap.isRelation = true;
              res.couponsMemberRelationMap.endTime = dateHelper.formatISODate(
                res.couponsMemberRelationMap.endTime,
                "YYYY-MM-DD"
              );
              ...
              if (this.prevData) {
                        ...
                        if (orderPrice >= afterDiscountPrice) {
                          ...
                          res.couponsMemberRelationMap.afterDiscountPrice =
                            res.couponsMemberRelationMap.discountAmount > 0
                              ? utilHelper.toFixed(unitPrice, 2)
                              : unitPrice;
                        }
                        res.couponsMemberRelationMap.isRelation = true;
                      });
                ...
                if (res.couponsMemberRelationMap.preferentialType == 2) {
                  if (this.prevData.fullPriceReductionAfter) {
                    if (
                      this.prevData.fullPriceReductionAfter >=
                      res.couponsMemberRelationMap.limitedPrice
                    ) {
                      //console.log(res.couponsMemberRelationMap.limitedPrice)
                      res.couponsMemberRelationMap.afterDiscountPrice =
                        this.prevData.fullPriceReductionAfter -
                        res.couponsMemberRelationMap.reducedPrice;
                    }
                    return;
                  }
                  if (
                    this.prevData.actualPrice >=
                    res.couponsMemberRelationMap.limitedPrice
                  ) {
                    res.couponsMemberRelationMap.afterDiscountPrice =
                      this.prevData.actualPrice -
            ...
            this.couponsList = result.data.records;
          }
        });
    },
      ...
      https
        ...
        .then((result) => {
          ...
          if (result.success) {
            ...
            this.couponsPointsList = result.data.records;
          }
        });
    },
    ...
    onClick(e) {
      ...
      ].couponsMemberRelationMap.isShow = this.couponsList[
        e.currentTarget.dataset.index
      ].couponsMemberRelationMap.isShow
        ? false
        : true;
    ...
    onPointsClick(e) {
      //设置当前点击的使用规则为显示
      this.couponsPointsList[
        e.currentTarget.dataset.index
      ].couponsMemberRelationMap.isShow = this.couponsPointsList[
        e.currentTarget.dataset.index
      ].couponsMemberRelationMap.isShow
        ? false
        : true;
      this.couponsPointsList = this.couponsPointsList;
    },
    ...
    onImmediateUse(e) {
      if (this.prevData) {
        ...
        afterDiscounts.id =
          this.couponsList[
            e.currentTarget.dataset.index
          ].couponsMemberRelationMap.id;
        //console.log(this.data.couponsList[e.currentTarget.dataset.index].couponsMemberRelationMap.afterDiscountPrice)
        afterDiscounts.price =
          this.couponsList[
            e.currentTarget.dataset.index
          ].couponsMemberRelationMap.afterDiscountPrice;
        afterDiscounts.couponsName =
          this.couponsList[
            e.currentTarget.dataset.index
          ].couponsMemberRelationMap.couponsName;
        afterDiscounts.preferentialTypeText = systemStatus.preferentialTypeText(
          this.couponsList[e.currentTarget.dataset.index].preferentialType
        );
        console.log(this.prevData);
        // prevPage.setData({
        ...
        // })
        this.afterDiscount = afterDiscounts;
        this.prevData.fullPriceReduction = utilHelper.toFixed(
          this.prevData.actualPrice - afterDiscounts.price,
          2
        );
        this.prevData.couponsIsHidden = true;
        this.afterDiscount = afterDiscounts;
        this.getFullReductionRule();
      ...
      if (this.couponsList[couponIndex].shopList.length == 1) {
        uni.navigateTo({
          url:
            "../../menu/index/index?id=" +
            this.couponsList[couponIndex].shopList[0].id,
        });
    ...
    onImmediatePointsUse(e) {
      if (this.prevData) {
        ...
        afterDiscounts.preferentialTypeText = systemStatus.preferentialTypeText(
          this.couponsPointsList[e.currentTarget.dataset.index].preferentialType
        );
        console.log(this.prevData);
        this.afterDiscount = afterDiscounts;
        this.prevData.fullPriceReduction = utilHelper.toFixed(
      ...
      https
        ...
        .then((result) => {
          ...
          if (result.success) {
            ...
            //获取配送费，配送费不作为满减条件
            let feeData = app.globalData.deliveryAndSelfTaking
            ...
            this.prevData.actualPrice = this.prevData.actualPrice
              ? utilHelper.toFixed(Number(this.prevData.actualPrice), 2)
              : 0;
            ...
            this.prevData.fullPriceReductionIsHidden =
              this.prevData.fullPriceReductionIsHidden;
            ...
            if (prevPage.data.reducedDeliveryPrice >= feeData) {
              ...
              reducedDeliveryPrice =
                feeData - prevPage.data.reducedDeliveryPrice;
            }
            console.log(
              "fullPriceReductionBefore===>" + fullPriceReductionBefore
            );
            console.log(
              "获取商家配送费====" + prevPage.data.reducedDeliveryPrice
            );
            console.log("获取用户地址配送费====" + feeData);
            console.log("获取商家支付后的配送费====" + reducedDeliveryPrice);
            console.log(
              "获取价格====" + this.prevData.fullPriceReductionBefore
            );
            console.log("获取优惠券信息===>");
            ...
            for (let i = 0; i < result.data.records.length; i++) {
              ...
              if (
                Number(fullPriceReductionBefore) >=
                result.data.records[i].limitedPrice
              ) {
                //判断当前满减价格limitedPrice和上一个满减价格对比，如果大于就进行认证
                if (
                  result.data.records[i].limitedPrice >
                  this.prevData.limitedPrice
                ) {
                  this.prevData.limitedPrice =
                    result.data.records[i].limitedPrice;
                  console.log(result.data.records[i].reducedPrice);
                  this.prevData.fullPriceReduction = utilHelper.toFixed(
                    fullPriceReductionBefore -
                      result.data.records[i].reducedPrice,
                    2
                  );
                  this.prevData.fullReductionRuleName =
                    result.data.records[i].name;
                  this.prevData.fullReductionRuleId = result.data.records[i].id;
                  this.prevData.fullPriceReductionIsHidden = true;
                  this.prevData.fullPriceReductionAfter = utilHelper.toFixed(
                    this.prevData.fullPriceReduction -
                      result.data.records[i].reducedPrice,
                    2
                  );
                }
            ...
            console.log(
              "获取满减后的金额===》" + this.prevData.fullPriceReduction
            );
            if (app.globalData.deliveryAndSelfTaking.currentTab == 0) {
              this.prevData.fullPriceReduction = utilHelper.toFixed(
                this.prevData.fullPriceReduction + reducedDeliveryPrice,
                2
              );
            }
      ...
      https
        ...
        .then((result) => {
          ...
          if (result.success) {
            ...
            this.prevData.fullPriceReductionAfter = this.prevData.actualPrice;
            this.prevData.discountPrice = 0;
            this.prevData.fullPriceReductionBefore =
              this.prevData.actualPrice > 0
                ? this.prevData.actualPrice - feeData
                : 0;
            this.prevData.fullPriceReductionIsHidden =
              this.prevData.fullPriceReductionIsHidden;
            this.prevData.limitedPrice = 0;
            ...
            for (let i = 0; i < result.data.records.length; i++) {
              //总价格减去配送费大于满减金额则进行满减操作
              if (
                Number(this.prevData.fullPriceReductionBefore) >=
                result.data.records[i].limitedPrice
              ) {
                //判断当前满减价格limitedPrice和上一个满减价格对比，如果大于就进行认证
                if (
            ...
            console.log(
              "获取最终优惠金额=====" + this.prevData.fullPriceReduction
            );
    ...
    onRadioChange(e) {
      ...
      this.couponsList[
        e.currentTarget.dataset.index
      ].couponsMemberRelationMap.checked = false;
      this.couponsList = this.couponsList;
    },
