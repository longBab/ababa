	...
	<view>
		<view class="position-sticky" id="top_tab_list">
			...
			<view>
				<uni-search-bar 
					@confirm="searchFun" 
					@cancel="searchInputClear" 
					@clear="searchInputClear"
					placeholder="请输入联系人手机号/订单号"
					cancelButton="cancel"
				/>
			</view>
					...
					<view class="swiper-tab-item two-tab" :data-current="index" @tap="clickOrderTab"
						...
						<view :class="'swiper_table_item_view ' + (currentOrderTab == item.modeId ? 'active' : '')"
							...
							<view class="badge_position" v-if="item.number > 0">
								// #ifdef APP-PLUS||H5
								<uni-badge :text="item.number > 0 ? item.number.toString() : ''" />
								// #endif
		...
		<block v-if="currentTab == 0">
			<scroll-view @scrolltoupper="onPullDownRefresh" upper-threshold="-50" :style="'height:' + winHeight + 'px;'"
				...
				<view class="divider" v-if="!isLoading&&isEndList">
					<text class="divider-text">没有更多啦</text>
				</view>
			</scroll-view>
			<Empty :description="'暂无'+shopOrderTabList[currentOrderTab].modeName+'订单'" v-if="list.length <= 0">
				<button class="uni-button" type="primary" size="mini" @tap="goToDrink">
					去逛逛
				</button>
			</Empty>
		</block>
		...
		<block v-if="currentTab == 1">
			<scroll-view @scrolltoupper="onPullDownRefresh" upper-threshold="-50" :style="'height:' + winHeight + 'px;'"
				...
				<view class="divider" v-if="!isMallLoading&&isEndMallList">
					<text class="divider-text">没有更多啦</text>
				</view>
			</scroll-view>
			<Empty :description="'暂无'+shopOrderTabList[currentOrderTab].modeName+'订单'" v-if="mallList.length <= 0">
				<button class="uni-button" type="primary" size="mini" @tap="goToDrink">
					去逛逛
				</button>
			</Empty>
...
<script>
	...
	export default {
			...
			authService.checkIsLogin().then((result) => {
				if (result) {
					...
					if (this.currentTab == 1) {
						console.log('如果pageNo��以pageSize不等于pageSize====' + pageMallNo * pageMallSize);
		...
		methods: {
			...
			selectTabNumber() {
				https.request('/rest/member/order/selectTabNumber', {}).then((result) => {
					if (result.success) {
						...
						this.shopOrderTabList = this.shopOrderTabList;
					}
				});
			},
			...
			selectMallTabNumber() {
				https.request('/rest/member/pointsMall/order/selectTabNumber', {}).then((result) => {
					if (result.success) {
						...
						console.log(this.shopOrderTabList);
						this.pointsOrderTabList = this.pointsOrderTabList;
					}
				});
			},
			...
			getOrderDetail() {
				https.request('/rest/member/order/selectById', {
					...
					if (result.success) {
						...
						this.list[index] = result.data.order;
			...
			onReachBottom() {
				console.log('占位：��数 onReachBottom 未声明');
				if (this.currentTab == 0) {
					if (this.isEndList) {
						this.isEndList = true;
						return;
