    ...
    <view>
        ...
        <view class="commdity-name-type-view">
            <view class="display_flex_between">
                ...
                <view class="flex_column collect-in">
                    <uni-icons :type="isCollect ? 'heart-filled' : 'heart'" size="24" :color="isCollect ? '#fc7a7c' : '#c3c3c3'"
                        @tap="parseEventDynamicCode(data.id, isCollect ? 'bindDeleteCollect' : 'bindInsertCollect')"
                        :data-goodsid="data.id"></uni-icons>
        ...
        <view class="commodity-detail-view">
            <view class="guige-yunfei">
                ...
                <text class="goods-info-specListString guige-specListString" :decode="true">{{ data.latelyMonthlySales
                    }}&nbsp;件</text>
            </view>
        </view>
        <view class="commodity-detail-view">
        ...
        <view class="shopping-cart-detail">
            <uni-popup ref="shoppingCartPopup" type="bottom">
                    ...
                    <scroll-view style="height: 55vh" scroll-y>
                        <view class="shoppingCart-item" v-for="(item, index) in shoppingCartList" :key="index">
                            ...
                            <view class="goodsPrice-number-view">
                                ...
                                <view class="stepper">
                                    <text class="reduce-class" @tap="bindMinus" :data-cartId="String(item.id)"
                                        :data-number="String(item.number)">－</text>
                                    <input disabled type="number" :value="String(item.number)" class="reduce-class" />
                                    <text @tap="bindPlus" class="add-class"
                                        :data-num="index + ',' + String(item.number)">＋</text>
                                </view>
                            </view>
                        </view>
                    </scroll-view>
            ...
            <uni-popup ref="specificationsPopup" type="bottom">
                <view class="content_box">
                    <view class="popup-header">
                    ...
                    <scroll-view scroll-y style="height: 50vh">
                        <view class="commdity-name-type-view">
                            <view class="commdity-type-item" v-for="(item, key) in specList" :key="key">
                                ...
                                <radio-group class="radio-group" @change="radioChange" :data-firstIndex="String(key)">
                                    <label :class="'group-label theme-border ' +
                                        ...
                                        <radio :value="String(index)" :checked="item.checked" :disabled="!item.stock"
                                            class="radio" />
                            ...
                            <Empty v-if="specList.length == 0" description="暂无数据"></Empty>
                        </view>
                    </scroll-view>
                    <view >
            ...
            <view class="buy-shopping-cart-view">
                <view class="cz-button">
                    <view class="immediate-purchase theme-bg" @tap="openSpecifications" data-type="pay"
                        ...
                        立即购买(￥{{ fullPriceReductionIsHidden ? String(fullPriceReduction) : String(data.price) }})
                    </view>
...
export default {
  ...
  onLoad: function (options) {
    ...
    methods: {
        parseEventDynamicCode(e, eventName) {
            this[eventName](e);
        },
        radioChange(e) {
            //获取复选框选中的下标值
            ...
            // 将 checkValue 转换回数字类型（如果需要）
            specList[firstIndex][parseInt(checkValue)].checked = true;
            ...
            for (let i = 0; i < this.fullReductionRuleList.length; i++) {
                if (totalPrice >= this.fullReductionRuleList[i].limitedPrice) {
                    if (this.fullReductionRuleList[i].limitedPrice > limitedPrice) {
                        ...
                        fullPriceReduction = totalPrice - this.fullReductionRuleList[i].reducedPrice;
            ...
            this.totalPrice =  String(Number(totalPrice).toFixed(2)); 
            this.fullPriceReduction = String(Number(fullPriceReduction).toFixed(2)); 
        ...
        toPay() {
            ...
            uni.navigateTo({
                url: '/pages/pay/pay?payType=' + this.chooseType + '&orderDetail=' + JSON.stringify(orderDetail)
            });
        },
        ...
        autoHeigth() {
            ...
            try {
                ...
                query.select('.shopping-cart-detail').boundingClientRect(data => {
                    if (data) {
                        this.winHeight = uni.getSystemInfoSync().windowHeight - (data.height + 5);
                    }
                }).exec();
            } catch (error) {
                console.error('autoHeight error:', error);
            }
        },
        ...
        getFullReductionRule(commodityDetails) {
            https.request('/rest/pointsMall/fullReductionRule/list', {
                ...
                if (result.success) {
                    ...
                    this.totalPrice = String(commodityDetails.price);
                }
            });
        },
        ...
        selectByGoodsId(goodsId) {
            https.request('/rest/pointsMall/goodsSpecificationOption/selectByGoodsId', {
                ...
                if (result.success && result.data) {
                    ...
                    this.specListString = specListString ? specListString : '暂无规格'
        ...
        chooseSpecifications() {
            ...
            authService.checkIsLogin().then((result) => {
                ...
                if (result) {
                    ...
                    // 直接赋值
                    _this.repData = JSON.stringify(goodsSpecs);
                    _this.specificationsDialog = false;

                    if (_this.chooseType == 'pay') {
                        _this.goToPay();
        ...
        getGoodsCollect(goodsId) {
            https.request('/rest/member/pointsMall/goodsCollect/selectByGoodsId', {
                ...
                if (result.success) {
                    ...
                    this.isCollect = result.data ? true : false
                    // this.setData({
