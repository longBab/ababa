<template>
  <view>
      ...
      <view class="position-sticky">
          ...
          <form action="/">
            ...
            >
              <template v-slot:right>
                <view @click="searchBtn" class="search-btn">搜索</view>
              </template>
            </uni-search-bar>
          </form>
        </view>
        <view class="swiper-content">
          ...
          <view
            ...
            v-if="!isShowCheckbox"
            @tap="bindShowCheckbox"
            >管理</view
          >
          <view
            class="manager-checkbox theme-color"
            v-else
            @tap="bindShowCheckbox"
            >完成</view
          >
        ...
        <view class="page__bd">
          ...
          >
            <van-swipe-cell :right-width="65">
              <view
                :class="'weui-slidecell ' + (item.disable ? 'isDisable' : '')"
              >
                ...
                >
                  ...
                  ></image>
                  <view class="sell-out out-store" v-if="item.goodsStatus == 1"
                    >未上架</view
                  >
                  <view
                    class="sell-out out-store"
                    v-else-if="item.goodsStatus == 3"
                    >已下架</view
                  >
                  <view
                    class="sell-out out-store"
                    v-else-if="item.goodsStatus == 4"
                    >已售罄</view
                  >
                  <view class="commdity-types">
                    ...
                    <view class="commdity-money strike_through" v-if="isSale"
                      >￥{{ item.goodsPrice }}
                    </view>
                    <view class="commdity-money">
                      ￥{{
                        item.isSale ? item.salePrice : item.goodsPrice
                      }}</view
                    >
                    <text class="is-goods-exists" v-if="!item.isGoodsExists"
                      >已失效</text
                    >
                  </view>
                </view>
              ...
              <template #right>
                <view style="height: 100%" class="flex_center">
                  <view
                    class="flvan-swipe-cell__right flex_between"
                    :data-checkboxIndex="index"
                    @tap="slideButtonTap($event, { goodsid: item.goodsId })"
                  >
                    <uni-icons type="star" size="30" color="red" />
                  </view>
                </view>
              </template>
              // #endif // #ifdef MP-WEIXIN||MP-ALIPAY
              <view
                slot="right"
                class="flvan-swipe-cell__right flex_between"
                :data-checkboxIndex="index"
    ...
    <view v-if="isShowCheckbox" class="safe-area show-checkbox">
      ...
      <uni-icons type="star" size="25" color="red" @click="batchDelete" />
    </view>
    <Empty description="暂无收藏" v-if="collectList.length <= 0"></Empty>
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    getPointsMallCollect(isBuy, goodsName) {
      ...
      this.collectList = [];
      https
        .request("/rest/member/pointsMall/goodsCollect/list", data)
        .then((result) => {
          if (result.success) {
    ...
    bindPointsMallDetails(e) {
      uni.navigateTo({
        url:
          "../../../internal/mall/detail/detail?id=" +
    ...
    checkboxChange(e) {
      ...
      this.isAlls = values.length != this.collectList.length ? false : true;
    },
    ...
    allChange(e) {
      ...
      this.collectList = this.collectList;
      this.goodsIdList = deleteIds;
      this.isAlls = this.isAlls ? false : true;
    },
    ...
    batchDelete() {
      ...
      toastService.showModal(
        null,
        "确定要取消这" + this.goodsIdList.length + "条收藏吗?",
        function confirm() {
