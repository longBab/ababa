			...
			<checkbox-group class="weui-slidecells" @change="checkboxChange">
				<view class="page__bd">
					<view class="weui-slidecells" v-for="(item, index) in items" :key="index">
						<uni-swipe-action>
							<uni-swipe-action-item :right-options="swipeOptions" @click="slideButtonTap($event, index)">
								<view :class="'weui-slidecell ' + (item.disable ? 'isDisable' : '')">
									...
									<view class="stepper">
										...
										<input disabled type="number" :value="item.number.toString()" class="radd-reduce-input" />
		...
		methods: {
			initData() {
				...
				for (let i = 0; i < items.length; i++) {
					...
					if (items[i].checked) {
						totalPrice += items[i].price * items[i].number;
					}
			...
			slideButtonTap(e, index) {
				...
				toastService.showModal(
					...
					function confirm() {
						that.updateNumber(id, number, 0, function callback() {
							totalPrice = totalPrice - items[index].price * number;
							items.splice(index, 1);
							that.items = items;
			...
			checkboxChange(e) {
				...
				for (var i in values) {
					const index = parseInt(values[i]); // 将字符串转回数字
					items[index].checked = true;
					totalPrice = totalPrice + items[index].price * items[index].number;
					fullPriceReduction = fullPriceReduction + items[index].price * items[index].number;
					totalNum = totalNum + items[index].number;
					packingCharges = packingCharges + items[index].packingCharges * items[index].number;
				}
				this.items = items;
				this.totalPrice = totalPrice;
			...
			bindMinus: function(e) {
				var that = this;
				let numList = e.currentTarget.dataset.num.split(','); // 现在得到的就是字符串
				let items = this.items;
				//点击减号后则当前商品的数量-1,如果这个商品不能被操作则return
				if (items[parseInt(numList[0])].disable) {
					return;
				}
				if (!items[parseInt(numList[0])].checked) {
					toastService.showToast('未选择商品');
					return;
				}

				// 使用 parseInt 确保进行数字运算
				let currentIndex = parseInt(numList[0]);
				let currentNumber = parseInt(numList[1]);
				
				//重新计算被选中的商品的总金额
				totalPrice = 0;
				let packingCharges = items[currentIndex].packingCharges; //获取该点击的商品的包装费
				for (let i = 0; i < items.length; i++) {
					if (items[i].checked) {
						//如果被选中则进行总额计算（已经计算出的价格+商品价格*商品的数量)
						if (currentIndex == i) {
							//console.log(number)
							totalPrice = totalPrice + items[i].price * currentNumber;
						} else {
				...
				if (currentNumber == 1) {
					toastService.showModal(null, '确定不要这个了吗？', function confirm() {
						that.updateNumber(items[currentIndex].id, 1, 0, function callback() {
							items.splice(currentIndex, 1); //列表减一
							totalNum--;
							that.items = items;
							that.totalPrice = totalPrice;
							that.packingCharges = that.packingCharges - packingCharges;
							that.getFullReductionRule();
							that.getAutoHeight();
						});
					});
				...
				items[currentIndex].number = currentNumber - 1;
				this.updateNumber(items[currentIndex].id, 1, 0, function callback() {
					totalNum--;
					that.items = items;
					that.totalPrice = totalPrice;
					that.packingCharges = that.packingCharges - packingCharges //所有商品包装费减去当前点击的商品的包装费
				});
			...
			bindPlus: function(e) {
				...
				items[currentIndex].number = currentNumber + 1;
				let packingCharges = items[currentIndex].packingCharges; //获取该点击的商品的包装费;
				this.updateNumber(items[currentIndex].id, 1, 1, function callback() {
					//重新计算被选中的商品的总金额
					for (let i = 0; i < items.length; i++) {
						if (items[i].checked) {
							//如果被选中则进行总额计算（已经计算出的价格+商品价格*商品的数量)
