        ...
        <block v-for="(item, index) in carouselUrls" :key="index">
          <swiper-item class="carousel-swiper-item">
            <image
              ...
              mode="aspectFill"
              :data-imageLinkUrl="item.imageLinkUrl"
              @tap="carouseCommodityDetailTap"
            />
          </swiper-item>
        </block>
      </swiper>
      ...
      >
        ...
        <view class="like-money-view">
          <view class="like-money">
            ...
            <text>
              {{ item.price }}
              <text class="score-money" :decode="true"
                >&nbsp;&nbsp;(可用积分等额兑换)</text
              >
            </text>
          </view>
        ...
        <view class="like-money-view">
          <text class="latelyMonthlySales out_of_range one_row"
            >已售：{{ item.latelyMonthlySales }}</text
          >
          <view
            class="insert-car"
            @tap.stop.prevent="openSpecifications"
            :data-index="index"
          >
            <uni-icons type="plus" size="24" class="icongouwuche1 theme-bg" />
          </view>
        </view>
      </view>
    </view>
    <view class="divider" v-if="isMore">
      <text class="divider-text">没有更多啦</text>
    </view>

    <uni-popup ref="popup" type="bottom" :show="specificationsDialog">
      <view class="popup-container">
        <view class="popup-content">
          <view class="content_box">
            ...
            <view class="goods-info-view">
              ...
              ></image>
              <view>
                <view class="goods-info-name out_of_range one_row">{{
                  goods.name
                }}</view>
        ...
        <view class="popup-footer">
          <view
            class="good-choice-btn theme-bg"
            @tap="insertShoppingCart"
      ...
      .then((result) => {
        ...
        if (result.success) {
          ...
          if (result.data.records.length <= 0) {
            pageNo = pageNo - 1;
            that.isMore = result.data.hasNextPage ? false : true;
          ...
          this.isMore = result.data.hasNextPage ? false : true;
        }
      });
  },
  /**
      ...
      https
        ...
        .then((result) => {
          if (result.success) {
            ...
            this.classifyList = result.data.records;
          }
        });
    },
    ...
    insertShoppingCart(e) {
      authService.checkIsLogin().then((result) => {
        ...
        if (result) {
          ...
          https
            .request(
              "/rest/member/pointsMall/shoppingCart/insert",
              data,
