<template>
    ...
    <view class="content-class">
      <view id="page-top-view">
        <view style="padding-bottom: 20rpx; background: #f6f6f6">
          <view
            class="flex_between public-views"
            style="background-color: white"
          >
            <view
              class="shop_info_view"
              @tap="
                parseEventDynamicCode($event, !memberInfo.id ? '' : 'selfTap')
              "
              :data-index="selfOutActiveIndex"
            >
              <view
                v-if="selfOutActiveIndex == 0 && !initShopInfo.id && !isLoading"
                class="flex_start"
                style="font-size: 30rpx"
              >
                <view>该区域外送功能即将开通</view>
                <uni-icons type="arrow-right" size="14"></uni-icons>
              </view>
              <block
                v-if="selfOutActiveIndex == 0 && initShopInfo.id && !isLoading"
              >
                <view @tap="selfTap" :data-index="selfOutActiveIndex">
                  ...
                  <view class="distance_m" v-if="initShopInfo">
                    距您{{
                      initShopInfo.shopAdditionalVo.deliveryDistanceText
                    }}</view
                  >
                </view>
              </block>
              <block
                v-if="
                  (selfOutActiveIndex == 0 || selfOutActiveIndex == 1) &&
                  initShopInfo &&
                  isLoading
                "
              >
              ...
              >
                <view @tap="selfTap" :data-index="selfOutActiveIndex">
                  <view class="flex_between">
                    <view class="out_of_range one_row">
                      {{ deliveryAndSelfTaking.deliveryAddress.province
                      }}{{ deliveryAndSelfTaking.deliveryAddress.city
              ...
              <block
                ...
                "
              >
                <view class="no_login_out_tip"> 暂不支持外送 </view>
              </block>
            </view>
            <view
              class="flex_between self_out_button"
              v-if="selfOutItems.length > 0 && !isLoading"
            >
              <view
                v-for="(item, index) in selfOutItems"
                @tap="selfTap"
                :data-index="item.index"
                :class="
                  selfOutActiveIndex == index
                    ? 'self_out_active self_button theme-bg'
                    : 'self_button'
                "
              >
                {{ item.text }}
      ...
      >
        <scroll-view
          ...
          :enable-flex="true"
          :style="
            'height:' + (winHeight - (carHeight ? carHeight : 70) - 5) + 'px;'
          "
          :scroll-into-view="'left' + activeLeftTab"
          :scroll-with-animation="true"
          class="menu_left_scorll"
        >
          <block v-for="(menu, menuIndex) in menuList">
            <view
              :id="'left' + menuIndex"
              v-if="menu.goodsList.length > 0"
              @tap="leftTap"
              :data-index="menuIndex"
              :class="
                menuIndex == activeLeftTab
                  ? 'menu_left_content theme-bg left_active '
                  : 'menu_left_content'
              "
            >
              <view class="menu_left_name">
                <view class="out_of_range one_row menu_left_name_text">{{
                  menu.name
                }}</view>
              </view>
            </view>
        ...
        <scroll-view
          ...
          :style="
            'height:' +
            (winHeight - (carHeight ? carHeight : 70) - 5) +
            'px;margin-left:1rpx;background: #f6f6f6;'
          "
          @scroll="mainScroll"
        ...
        >
          <view class="menu_right_scorll_view">
            ...
            >
              <view
                class="commodity-type position-sticky"
                v-if="menu.goodsList.length > 0"
              >
              ...
              <block
                v-for="(chil, goodsIndex) in menu.goodsList"
                :key="goodsIndex"
              >
                <view
                  :class="
                    'commodity-item ' + (chil.goodsStatus == 4 ? 'isEnd' : '')
                  "
                  :hover-class="
                    chil.goodsStatus == 4 ? '' : 'hover-class-public'
                  "
                >
                  <view
                    class="flex_between commodity-name-view"
                    @tap="
                      parseEventDynamicCode(
                        $event,
                        chil.goodsStatus == 4 ? '' : 'commodityDetailTap'
                      )
                    "
                  ...
                  >
                    <image
                      :src="
                        chil.mainImage
                          ? chil.mainImage
                          : '/static/assets/images/load-image.png'
                      "
                      mode="aspectFill"
                      class="commodity-image"
                    ></image>
                    <view class="sell-out" v-if="chil.goodsStatus == 4"
                      >售罄</view
                    >
                      ...
                      <view class="flex_between money-view">
                        ...
                        >
                          <block v-if="chil.number > 0">
                            <view class="stepper">
                              <view
                                class="flex_center insert-view reduce-class"
                                @click.stop="bindMinus"
                                :data-cartId="chil.cartId"
                          ...
                          <block v-else>
                            <view
                              :class="
                                'insert-view theme-bg ' +
                                (chil.goodsStatus == 4 ? 'isEnd' : '')
                              "
                              :data-goodsId="chil.goodsId"
                              @click.stop="
                                parseEventDynamicCode(
              ...
              <view v-if="menuList.length - 1 == menuIndex" class="more_box"
                >没有更多啦~</view
              >
            </view>
          </view>
        ...
        <view
          id="menu_scorll_mask"
          :class="ifScroll ? '' : 'menu_scorll_mask'"
        ></view>
        <view
          class="flex_column shopping-cart-detail"
          id="shopping-cart-detail"
        >
          ...
          >
            ...
            >
              <view>
                <uni-icons
                  ...
                  size="24"
                  :class="
                    'iconfont icongouwuche1 highlight ' +
              ...
              <view>
                <view class="shopping-cart-totalPrice">
                  ...
                  <view
                    class="full-price-reduction"
                    v-if="
                      fullPriceReductionIsHidden && shoppingCartList.length > 0
                    "
                  >
                    ￥{{ fullPriceReduction }}</view
                  >
                  <view
                    class="not-full-price-reduction"
                    v-if="shoppingCartList.length <= 0"
                    >暂未选购商品</view
                  >
                </view>
            ...
            <view
              :class="
                (isStartDeliveryPrice ? 'theme-bg' : 'shopping-cart-bg') +
                ' shopping-cart-right'
              "
              @tap="
                parseEventDynamicCode(
      ...
      <Empty
        description="暂无商品"
        v-if="menuList.length <= 0 && !isLoading"
      ></Empty>
      <!-- 替换购物车弹出层 -->
      <uni-popup
      ...
      >
        <view class="action-sheet">
          <view class="action-sheet-header">
            <text class="action-sheet-title">已选商品</text>
          </view>
            ...
            <scroll-view style="height: 55vh" scroll-y>
              ...
              >
                <view class="goodsName-restructure-view">
                  <view class="goodsName out_of_range one_row">{{
                    item.goodsName
                  }}</view>
                  <view class="restructure out_of_range one_row">{{
                    item.restructure
                  }}</view>
                </view>
                ...
                <view class="goodsPrice-number-view">
                  ...
                  <view class="stepper">
                    <view
                      class="flex_center car_reduce_add reduce-class"
                      @tap="bindMinus"
                      :data-cartid="item.id"
                    ...
                    <input
                      ...
                      type="number"
                      :value="item.number"
                      class="add-reduce-input"
                    />
                    <view
                      @tap="bindPlus"
                      class="flex_center car_reduce_add add-class"
                      :data-num="index + ',' + item.number"
                      >＋</view
                    >
                  </view>
                </view>
      ...
      <!-- 替换规格选择弹出层 -->
      <uni-popup
        ref="specificationsPopup"
        type="bottom"
        @close="closeSpecifications"
      >
        <view class="action-sheet">
          <view class="action-sheet-header">
            <text class="action-sheet-title">选择规格</text>
          </view>

          <view class="content">
            <view class="goods-info-view">
              <image
                :src="goodsInfo.mainImage"
                mode="aspectFill"
                class="commodity-image"
              ></image>
              <view>
                <view class="goods-info-name">{{ goodsInfo.name }}</view>
                <view class="goods-info-specListString"
                  >已选：{{ specListString }}</view
                >
            ...
            <scroll-view scroll-y style="height: 50vh">
              <view class="commdity-name-type-view">
                ...
                <view
                  class="commdity-type-item"
                  v-for="(item, key) in specList"
                  :key="key"
                >
                ...
                <view
                  class="loading_box"
                  v-if="specLoading && specList.length == 0"
                >
                  <uni-load-more
                    status="loading"
                    :iconSize="16"
            ...
            <view slot="footer" class="position-sticky-bottom">
              <view class="good-choice-btn theme-bg" @tap="insertShoppingCart"
                >我选好了</view
              >
            </view>
          </view>
  ...
  >
      ...
      <scroll-view style="height: 55vh" scroll-y>
        <view v-if="shopInfo.promotionList.length > 0">
          ...
          <view class="business-discount-info business-info-flex">
            <view
              class="business-discount out_of_range one_row"
              @tap="isPromotionTap"
            >
              <view
                class="theme-color-border business-discount-list"
                v-if="index <= 5"
                v-for="(rule, index) in shopInfo.promotionList"
                :key="index"
              >
                {{ rule.name }}
              </view>
        ...
        <view
          class="reduced-delivery-price"
          v-if="shopInfo.shop.reducedDeliveryPrice"
        >
    ...
    <uni-popup ref="outOfRangePopup" type="bottom" @close="close">
      <view class="action-sheet">
        <view class="action-sheet-header">
          <text class="action-sheet-title">本店超出配送范围</text>
        </view>
        ...
        <view class="action-sheet-footer">
          <button
            size="mini"
            class="edit-address-btn theme-bg"
            @tap="editAddress"
...
export default {
  ...
  onShow: function () {
    ...
    if ("selfOutActiveIndex" in app.globalData.deliveryAndSelfTaking) {
      selfOutActiveIndex =
        app.globalData.deliveryAndSelfTaking.selfOutActiveIndex;
    }
    ...
    this.deliveryAndSelfTaking.deliveryAddress =
  ...
  methods: {
    ...
    closeShoppingCart: function () {
      this.$refs.shoppingCartPopup.close();
    },
      ...
      authService.checkIsLogin().then((result) => {
        if (result) {
          if (this.shoppingCartList.length > 0) {
            this.$refs.shoppingCartPopup.open();
          }
          return;
        }
        _this.checkIsAuth();
    ...
    clickTab: function (e) {
      var that = this;
      this.$refs.shoppingCartPopup.close();
      this.$refs.specificationsPopup.close();
    ...
    autoHeight() {
      ...
      query
        ...
        .boundingClientRect(function (rect) {
          console.log(rect);
          let height =
            windowHeight > 630 ? rect[0].height - 10 : rect[0].height + 25;
          that.winHeight = windowHeight - height;
          that.leftHeight = windowHeight - height;
          that.bussinessHeight = windowHeight - height;
        })
        .exec();
    },
    ...
    selectMenuTap(e) {
      ...
      console.log(index);
      this.toView = "NAV" + index.toString();
      this.currentActiveIndex = index;
    },
        ...
        .then((result) => {
          if (result.success) {
            if (result.data.records.length == 0) {
              toastService.showModal("", "暂无配送地址", function () {
                _this.selfOutActiveIndex = 0;
              });
            }
            if (result.data.records.length > 0) {
    ...
    getMenuList(shopId) {
      var _this = this;
      https
        .request("/rest/menu/listWithGoods", {
          shopId: shopId,
        })
        .then((result) => {
          if (result.success && result.data) {
      ...
      https
        ...
        .then((result) => {
          if (result.success && result.data) {
                ...
                result.data.records.forEach((result, carIndex) => {
                  if (goods.goodsId == result.goodsId) {
                    ...
                    this.menuList[menuIndex].goodsList[goodsIndex].number =
                      number;
                    this.menuList[menuIndex].goodsList[goodsIndex].cartId =
                      result.id;
                  }
                });
              });
            ...
            result.data.records.forEach((result, index) => {
              ...
              for (var key in JSON.parse(result.specList)) {
                specList =
                  ...
                  JSON.parse(result.specList)[key];
              }
              result.restructure = specList;
              totalNum = totalNum + result.number;
              result.goodsPrices = utilHelper.toFixed(
                Number(result.goodsPrice) * result.number,
                2
              );
              totalPrice += result.price * result.number; //初始化被选中的商品的总金额
              result.disable =
                result.goodsStatus == 1 ||
                result.goodsStatus == 3 ||
                result.goodsStatus == 4
                  ? true
                  : false;
              packingCharges =
                result.goodsStatus == 1 ||
            ...
            this.shopInfo.shop.startDeliveryPrice = this.shopInfo.shop
              .startDeliveryPrice
              ? this.shopInfo.shop.startDeliveryPrice
              : 0;
            if (
            ...
            this.totalPrice = utilHelper.toFixed(
              totalPrice + packingCharges,
              2
            );
    ...
    getShopCartFullReductionRule() {
      https
        .request("/rest/fullReductionRule/list", {
          pageNo: -1,
          pageSize: 1,
            ...
            for (let i = 0; i < result.data.records.length; i++) {
              if (this.totalPrice >= result.data.records[i].limitedPrice) {
                if (result.data.records[i].limitedPrice > limitedPrice) {
                  ...
                  fullPriceReduction =
                    this.totalPrice - result.data.records[i].reducedPrice;
    ...
    appraiseTap(e) {
      ...
      this.appraiseIndex = appraiseIndex;
      this.appraiseJson = appraiseJson;
      this.content = appraiseId == appraiseJson.appraiseId ? this.content : "";
      this.appraiseFocus = true;
    ...
    getAppraiseTextareaHeight() {
      ...
      query.exec(function (res) {
        console.log();
        that.appraiseHeight = res[0].height;
      });
    ...
    handleCurrencyTap(e, _dataset) {
      ...
      /* ---处理dataset end--- */
      console.log(e);
      this.appraiseId = e.currentTarget.dataset.appraiseid;
      this.appraiseIndex = e.currentTarget.dataset.appraiseindex;
      ...
      https
        ...
        .then((result) => {
          ...
          if (result.success) {
            ...
            this.appraiseList = this.appraiseList;
          }
        });
    },
    ...
    openSpecifications(e) {
      this.$refs.specificationsPopup.open();
          ...
          if (result.success && result.data) {
            ...
            for (let key in specList) {
              ...
              for (let keyof in specList[key]) {
                //拼接查询规格等的json数据格式，查询商品规格等��应的价格
                specList[key][keyof].checked = false;
                //设置每个规格的第一个选项为选中，当库存为0时则选中���一个
    ...
    radioChange(e) {
      ...
      for (let i = 0; i < this.shopInfo.fullReductionRuleList.length; i++) {
        ...
        if (totalPrice >= this.shopInfo.fullReductionRuleList[i].limitedPrice) {
          if (
            this.shopInfo.fullReductionRuleList[i].limitedPrice > limitedPrice
          ) {
            limitedPrice = this.shopInfo.fullReductionRuleList[i].limitedPrice;
            fullPriceReduction =
              totalPrice +
              this.data.packingCharges -
              this.shopInfo.fullReductionRuleList[i].reducedPrice;
    ...
    insertShoppingCart(e) {
      ...
      authService.checkIsLogin().then((result) => {
        ...
        if (result) {
          ...
          toastService.hideLoading();
          https
            .request("/rest/member/shoppingCart/insert", {
    ...
    editAddress() {
      ...
      uni.navigateTo({
        url:
          "../../address/replace/replace?jump_page=menu_index&addressType=true&shopId=" +
          this.shopInfo.shop.id,
      });
    },
    ...
    topViewHeight() {
      ...
      query
        ...
        .boundingClientRect(function (rect) {
          console.log(rect);
          _this.pageTopView = rect[0].height * 2;
    ...
    selfTap(e) {
      ...
      var chooseId = this.deliveryAndSelfTaking.deliveryAddress
        ? this.deliveryAndSelfTaking.deliveryAddress.id
        : "";
      ...
      var shopId =
        selfOutActiveIndex == 0
          ? this.shopInfo.shop.id
          : app.globalData.deliveryAndSelfTaking.shopId;
    ...
    goTop(e) {
      ...
      this.$nextTick(() => {
        setTimeout(() => {
          _this.topRightNumer = 0;
          _this.topLeftNumer = 0;
          _this.activeLeftTab = 0;
          _this.activeTab = 0;
          setTimeout(() => {
            _this.getElementTop();
          }, 1000);
