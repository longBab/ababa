				...
				<radio-group class="radio-group-address" @change="radioChangePayment">
					<block v-for="(item, index) in paymentModes" :key="index">
						<label v-if="item.show"
							...
							<radio :value="String(index)" :checked="item.checked" class="pay_radio" />
		...
		<uni-popup ref="paymentPopup" type="bottom">
			<view class="popup-content">
				...
				<view class="popup-body">
					<view v-for="(item, index) in paymentModes" 
							:key="index"
							@tap="goToPay(item)"
							v-if="item.show"
							class="popup-item">
						...
						<radio-group class="radio-group-address" @change="radioChangeAddress">
							<label class="radio-label-address" v-for="(item, index) in addressList" :key="index">
								<block v-if="deliveryAndSelfTaking.deliveryAddress">
									<radio :value="String(index)" :checked="item.id == deliveryAndSelfTaking.deliveryAddress.id"
										class="radio" />
								</block>
								<block v-else>
									<radio :value="String(index)" class="radio" />
								</block>
								...
								<view class="flex_between choose-address-dialog">
									...
									<view>
										<text class="iconfont icon-edit" @tap="editAddress" :data-data="item" :data-key="index"></text>
									</view>
								</view>
							</label>
						</radio-group>
						<Empty v-if="addressList.length <= 0" description="暂无地址"></Empty>
					</scroll-view>
		...
		<uni-popup ref="passwordPopup" type="center" :mask-click="false">
			<view class="password-popup">
				<view class="flex_between content_box">
					<view></view>
					<view>输入支付密码</view>
					<text class="close-icon" @tap="balancePayFail">×</text>
				</view>
				// #ifdef APP-PLUS||H5
				<!-- 密码输入框 -->
				<view class="content_box" style="padding: 0 0;">
					...
					<uni-easyinput type="password" v-model="pwdVal" :focus="payFocus" @focus="payFocus = true" maxlength="6" />
				...
				<view class="content_box" style="padding: 0 16px;">
					<view class="password_dialog_tip">
						<text>使用会员卡余额支付需要验证身份，验证通过后才可进行支付。</text>
					</view>
			...
			radioChangePayment(e) {
				...
				authService.getOpenId().then((openId) => {
					...
					if (index == 1) {
						...
						if (!paymentModes[index].isBindTap) {
							toastService.showToast(paymentModes[index].desc);
							return;
						}
					}
					...
					paymentModes[index].checked = true;
					_this.paymentModes = paymentModes;
					_this.paymentModeIndex = index;
				});
			},
